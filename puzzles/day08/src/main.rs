...
fn second_solution(input: &str) -> u32 {
    ...
    lines.iter().map(|l| l.solve_output()).sum()
}
...
impl SevenString {
    ...
    fn sub(&self, rhs: &Self) -> Vec<char> {
        ...
        self.0.chars().filter(|c| !rhs.contains(c)).collect()
    }
}
...
impl Line {
    fn from(str: &str) -> Result<Self, String> {
        ...
        parser::line(str).map_err(|err| format!("Error while parsing input: {}, err: {}", str, err))
    }
    ...
    fn determine_positions(&self) -> SevenDisplay {
        ...
        let five_digits = self
            .input
            .iter()
            .filter(|ss| ss.len() == 5)
            .collect::<Vec<_>>();
        let three = five_digits
            .iter()
            .filter(|ss| ss.0.contains(x0) && ss.0.contains(x2) && ss.0.contains(x5))
            .next()
            .unwrap();
        let x3 = four
            .0
            .chars()
            .filter(|c| *c != x2 && *c != x5 && three.0.contains(*c))
            .next()
            .unwrap();
        let x1 = four
            .0
            .chars()
            .filter(|c| *c != x2 && *c != x3 && *c != x5)
            .next()
            .unwrap();
        let x6 = three
            .0
            .chars()
            .filter(|c| *c != x0 && *c != x2 && *c != x3 && *c != x5)
            .next()
            .unwrap();
        let x4 = eight
            .0
            .chars()
            .filter(|c| *c != x0 && *c != x1 && *c != x2 && *c != x3 && *c != x5 && *c != x6)
            .next()
            .unwrap();
